(function() {
  var CONFIG, Neat, PAGES_DIR, PAGES_TEMP_DIR, Q, TASK_DIR, applyLayout, asyncErrorTrap, checkGitStatus, compileStylus, createPages, createTempDir, cup, currentBranch, ensurePath, error, exec, find, findMarkdownFiles, findSiblingFileSync, findTitle, fs, getGitInfo, green, hasUnstagedChanges, hasUntrackedFile, highlight, info, loadConfig, marked, neatTask, parallel, puts, read, readFiles, red, relative, resolve, run, t, writeFiles, _ref, _ref1, _ref2, _ref3,
  ...
  applyLayout = function(files) {
    return loadConfig().then(function(config) {
      var content, dir, footer, fsf, hamlc, header, layout, navigation, newFiles, path, paths, rfs;
  ...
  createPages = function() {
    return findMarkdownFiles().then(read).then(function(files) {
      ...
      return newFiles;
    }).then(applyLayout).then(writeFiles).then(compileStylus);
  };
  ...
  exports['github:pages'] = neatTask({
    ...
    action: function(callback) {
      ...
      }).then(createTempDir).then(function(stdout) {
        return run("cp -r " + Neat.root + "/docs " + PAGES_TEMP_DIR + ";           rm -rf docs");
  ...
  exports['github:pages:preview'] = neatTask({
    ...
    action: function(callback) {
      ...
      git = null;
      branch = null;
      return createTempDir().then(function(stdout) {
        return run("cp -r " + Neat.root + "/docs " + PAGES_TEMP_DIR + ";           rm -rf docs");
      }).then(createPages).then(function() {
        return typeof callback === "function" ? callback(0) : void 0;
      }).fail(function(err) {
