...
(function() {
  var CLASS_MEMBER_RE, CLASS_RE, EXPORTS_RE, HASH_KEY_RE, HASH_RE, HASH_VALUE_RE, LITERAL_RE, MEMBER_RE, Neat, OBJECT_RE, REQUIRE_RE, SPLIT_MEMBER_RE, STRING_RE, analyze, annotateClass, annotateFile, cleanPath, coffee, compile, exportsToPackage, join, parallel, saveToFile, stripRequires, writeFile;
  ...
  analyze = function(path, content) {
    var comment, curClass, i, i2, line, m, out, p, s, _i, _len, _ref, _ref1;
    out = content.concat();
    i2 = 0;
    curClass = null;
    for (i = _i = 0, _len = content.length; _i < _len; i = ++_i) {
      ...
      if (CLASS_RE().test(line)) {
        _ref = CLASS_RE().exec(line), m = _ref[0], curClass = _ref[1];
