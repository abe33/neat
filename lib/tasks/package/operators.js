(function() {
  var CLASS_MEMBER_RE, CLASS_RE, EXPORTS_RE, HASH_KEY_RE, HASH_RE, HASH_VALUE_RE, LITERAL_RE, MEMBER_RE, NAME_RE, Neat, OBJECT_RE, PACKAGE_RE, REQUIRE_RE, SPLIT_MEMBER_RE, STATIC_MEMBER_RE, STRING_RE, analyze, annotateClass, annotateFile, cleanPath, coffee, compile, createDirectory, createFile, ensurePath, exportsToPackage, initValidate, join, malformedConf, parallel, parser, pathChange, pathReset, preventMissingConf, pro, resolve, rm, stripRequires, uglify, validate, writeFile, _, _ref, _ref1,
    _this = this;
  ...
  analyze = function(path, content) {
    ...
    for (i = _i = 0, _len = content.length; _i < _len; i = ++_i) {
      ...
      if (CLASS_RE().test(line)) {
        _ref2 = CLASS_RE().exec(line), m = _ref2[0], curClass = _ref2[1];
  ...
  validate('name', NAME_RE(), _('neat.tasks.package.expected_name'), preventMissingConf('name', join = function(buffer, conf, callback) {
    var k, newBuffer, newContent, newPath;
    newBuffer = {};
  ...
  createFile = function(buffer, conf, callback) {
    var gen, k, v;
  ...
  preventMissingConf('path', pathChange = function(buffer, conf, callback) {
    var content, newBuffer, path, rel;
    newBuffer = {};
    for (path in buffer) {
      content = buffer[path];
  ...
  preventMissingConf('path', pathReset = function(buffer, conf, callback) {
    var path;
