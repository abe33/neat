        ...
        return ensure(dir, asyncErrorTrap(function() {
          return find('cup', conf, asyncErrorTrap(function(files) {
            return readFiles(files, function(err, res) {
              var c, commands, ops, p;
              ops = [op.stripRequires, op.annotate, op.join, op.exportsToPackage, op.saveToFile, op.compile, op.saveToFile];
              commands = (function() {
                var _results;
                _results = [];
                for (p in res) {
                  c = res[p];
                  _results.push(Packager.asCommand(read(c), ops));
                }
                return _results;
              })();
