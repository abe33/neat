(function() {
    ...
    return exports[name] = neatTask({
      ...
      action: function(callback) {
        ...
        for (_i = 0, _len = watches.length; _i < _len; _i++) {
          w = watches[_i];
          recursiveWatch(path.resolve('.', w), watcher);
        }
        if (watches.length === 1) {
          return info(green(_('neat.tasks.watch.watching_singular', {
            dir: watches
          })));
        } else {
          return info(green(_('neat.tasks.watch.watching_plural', {
            dirs: watches
          })));
        }
      }
  ...
  watchTaskGen('watch:test:unit', 'test:unit', _('neat.tasks.watch_test_unit.description'), 'src', 'test/units');

  watchTaskGen('watch:test:functional', 'test:functional', _('neat.tasks.watch_test_functional.description'), 'src', 'test/functionals');

  watchTaskGen('watch:test:integration', 'test:integration', _('neat.tasks.watch_test_integration.description'), 'src', 'test/integrations');
