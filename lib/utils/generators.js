(function() {
  var Neat, describe, ensurePathSync, error, exists, fs, green, hashArguments, info, missing, multiEntity, namedEntity, noExtension, notOutsideNeat, path, render, resolve, usages, _, _ref, _ref1, _ref2,
    __slice = [].slice;
  ...
  multiEntity = function(src, entities, ctx, requireNeat) {
    ...
    return function() {
      var a, args, cb, generator, name, options, _i;
      generator = arguments[0], name = arguments[1], args = 4 <= arguments.length ? __slice.call(arguments, 2, _i = arguments.length - 1) : (_i = 2, []), cb = arguments[_i++];
      if (requireNeat) {
        if (Neat.root == null) {
          throw new Error(notOutsideNeat(process.argv.join(' ')));
      ...
      return entities.each(function(k, v) {
        ...
        dir = resolve(Neat.root, "" + dir + "/" + (a.join('/')));
        path = resolve(dir, "" + name + ext);
        partial = partial != null ? resolve(noExtension(src), partial) : src;
        context = ctx.concat();
