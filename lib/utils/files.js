(function() {
  var basename, dirWithIndex, dirWithIndexSync, ensure, ensurePath, ensurePathSync, ensureSync, error, extname, find, findBase, findBaseSync, findOnce, findSiblingFile, findSiblingFileSync, findSync, findSyncOnce, fs, isNeatRoot, isNeatRootSync, missing, neatRoot, neatRootSync, noExtension, parallel, puts, readFiles, readFilesSync, relative, resolve, rm, rmSync, touch, touchSync, warn, _ref, _ref1,
    __slice = [].slice,
  ...
  dirWithIndex = function(dir, ext, callback) {
    ...
    return fs.exists(dir, function(b) {
      var index;
      if (!b) {
  ...
  findOnce = function(pattern, ext, path, callback, output) {
    return fs.exists(path, function(b) {
          ...
          return fs.lstat(path, function(err, stats) {
            ...
            if (stats.isDirectory()) {
              return find(pattern, ext, path, function(err, files) {
                ...
                return fs.exists(index, function(b) {
  ...
  findSyncOnce = function(pattern, ext, path) {
    ...
    for (_i = 0, _len = content.length; _i < _len; _i++) {
      ...
      if (stats.isDirectory()) {
        ...
        if ((pattern != null) && p.match(pattern) && fs.existsSync(index)) {
          if (found == null) {
            found = [];
          }
          found.push(index);
  ...
  isNeatRoot = function(dir, callback) {
    return fs.exists(resolve(dir, ".neat"), callback);
  };

  isNeatRootSync = function(dir) {
    return fs.existsSync(resolve(dir, ".neat"));
  };
  ...
  rm = function(path, callback) {
    ...
    return fs.exists(path, function(exist) {
      if (exist) {
        return fs.lstat(path, function(err, stats) {
          if (err != null) {
            return typeof callback === "function" ? callback(err) : void 0;
