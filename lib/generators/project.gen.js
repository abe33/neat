(function() {
  ...
  usages('neat generate project <name> {description, author, keywords}', describe(_('neat.commands.generate.project.description'), project = function() {
    ...
    ensureSync(path);
    dirs = ["config", "config/packages", "lib", "src", "src/commands", "src/config", "src/config/environments", "src/config/initializers", "src/config/initializers/commands", "src/generators", "src/tasks", "templates", "test", "test/fixtures", "test/helpers", "test/functionals", "test/integrations", "test/units"];
