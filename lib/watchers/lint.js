(function() {
  var CLIWatchPlugin, Lint, Neat, Q, commands, error, findSiblingFile, green, info, puts, red, _ref, _ref1,
  ...
  findSiblingFile = Neat.require('utils/files').findSiblingFile;
  ...
  Lint = (function(_super) {
    ...
    Lint.prototype.runLint = function(paths) {
      ...
      findSiblingFile(p, Neat.paths, d, 'json', function(e, c, p) {
        var coffeelint;

        if (e != null) {
          return _this.deferred.reject(e);
        }
        coffeelint = Neat.resolve('node_modules/.bin/coffeelint');
        return _this.process = commands.run(coffeelint, ['-f', c].concat(paths), function(status) {
          return _this.handleStatus(status);
        });
      });
      return this.deferred.promise;
